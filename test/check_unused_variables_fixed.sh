...
for test_suite in "${TEST_SUITES[@]}"; do
    if [ -d "$test_suite" ]; then
        ...
        # Count warnings - fix the issue by using proper line counting
        WARNING_COUNT=$(echo "$BUILD_OUTPUT" | grep -c -i "unused\|warning" 2>/dev/null || echo "0")
        
        # Ensure WARNING_COUNT is a single integer
