...
./geographic_module_tests --gtest_output=xml:../$TEST_RESULTS_DIR/geographic_module_basic_tests.xml
...
cppcheck --enable=all --std=c++17 --xml --xml-version=2 \
    --output-file=../$TEST_RESULTS_DIR/geographic_module_cppcheck.xml \
    --suppress=missingIncludeSystem \
    --suppress=unusedFunction \
    --suppress=unmatchedSuppression \
    ../../client/mumble-plugin/lib/terrain_elevation.cpp \
    ../../client/mumble-plugin/lib/terrain_environmental_api.cpp \
    ../../client/mumble-plugin/lib/vehicle_dynamics.cpp

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ CppCheck completed for geographic module${NC}"
else
    echo -e "${YELLOW}⚠ CppCheck found issues in geographic module (see report)${NC}"
fi
...
clang-tidy -checks='*' -header-filter='.*' \
    ...
    ../../client/mumble-plugin/lib/terrain_environmental_api.cpp \
    ../../client/mumble-plugin/lib/vehicle_dynamics.cpp \
    -- -std=c++17 -I../../client/mumble-plugin/lib > ../$TEST_RESULTS_DIR/geographic_module_clang-tidy.txt

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ Clang-Tidy completed for geographic module${NC}"
else
    echo -e "${YELLOW}⚠ Clang-Tidy found issues in geographic module (see report)${NC}"
fi
...
valgrind --tool=memcheck \
    ...
    --track-origins=yes \
    --xml=yes \
    --xml-file=../$TEST_RESULTS_DIR/geographic_module_valgrind.xml \
    ./geographic_module_tests --gtest_output=xml:../$TEST_RESULTS_DIR/geographic_module_valgrind_tests.xml

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ Valgrind analysis completed for geographic module${NC}"
else
    echo -e "${YELLOW}⚠ Valgrind found memory issues in geographic module (see report)${NC}"
fi
...
./geographic_module_tests_asan --gtest_output=xml:../$TEST_RESULTS_DIR/geographic_module_asan_tests.xml
...
./geographic_module_tests_tsan --gtest_output=xml:../$TEST_RESULTS_DIR/geographic_module_tsan_tests.xml
...
./geographic_module_tests_coverage --gtest_output=xml:../$TEST_RESULTS_DIR/geographic_module_coverage_tests.xml

echo "Generating coverage report for geographic module..."
lcov --capture --directory . --output-file ../$COVERAGE_DIR/geographic_module_coverage.info
lcov --remove ../$COVERAGE_DIR/geographic_module_coverage.info '/usr/*' --output-file ../$COVERAGE_DIR/geographic_module_coverage_filtered.info
genhtml ../$COVERAGE_DIR/geographic_module_coverage_filtered.info --output-directory ../$COVERAGE_DIR/geographic_module_html
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${YELLOW}⚠ Coverage report generation failed for geographic module${NC}"
fi
...
time ./geographic_module_tests --gtest_filter="*Performance*" > ../$TEST_RESULTS_DIR/geographic_module_performance.txt 2>&1

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ Performance tests completed for geographic module${NC}"
else
    echo -e "${YELLOW}⚠ Performance tests had issues for geographic module${NC}"
fi
...
for i in {1..5}; do
    ...
    ./geographic_module_tests --gtest_filter="*Stress*" --gtest_repeat=10 > ../$TEST_RESULTS_DIR/geographic_module_stress_$i.txt 2>&1
done
...
cat > ../$TEST_RESULTS_DIR/geographic_module_test_report.html << EOF
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <div class="section">
        <h2>Code Coverage</h2>
        <p>Coverage report: <a href="../coverage/geographic_module_html/index.html">HTML Coverage Report</a></p>
    </div>
    ...
    <div class="section">
        ...
        <pre>$(cat ../$TEST_RESULTS_DIR/geographic_module_performance.txt 2>/dev/null || echo "Performance test log not available")</pre>
    </div>
</body>
</html>
EOF
