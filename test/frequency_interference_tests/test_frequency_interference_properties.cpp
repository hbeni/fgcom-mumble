...
TEST(PropertyTests, FrequencyRangePropertyTest) {
    rc::check("Frequency should be within radio range", []() {
        ...
        int freq_int = *rc::gen::inRange(1000000, 1000000000); // 1 MHz to 1 GHz
        double frequency = static_cast<double>(freq_int);
...
TEST(PropertyTests, ChannelSeparationPropertyTest) {
    rc::check("Channel separation should be positive", []() {
        ...
        int sep_int = *rc::gen::inRange(1, 1000000); // 1 Hz to 1 MHz
        double separation = static_cast<double>(sep_int);
