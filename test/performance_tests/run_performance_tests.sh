...
if cppcheck --enable=all --std=c++17 --suppress=missingIncludeSystem \
    ...
    echo -e "${YELLOW}⚠${NC} CppCheck completed for performance with warnings"
fi
...
if clang-tidy -checks='modernize-*,readability-*,performance-*,cppcoreguidelines-*' -header-filter='client/mumble-plugin/lib/.*' \
    ...
    echo -e "${YELLOW}⚠${NC} Clang-Tidy completed for performance with warnings"
fi
...
if timeout 300 valgrind --leak-check=full --show-leak-kinds=all \
    ...
    echo -e "${YELLOW}⚠${NC} Valgrind analysis completed for performance with issues"
fi
...
if ./performance_tests_asan; then
    ...
    echo -e "${YELLOW}⚠${NC} AddressSanitizer tests completed for performance with issues"
fi
...
if timeout 300 ./performance_tests_tsan; then
    ...
    echo -e "${YELLOW}⚠${NC} ThreadSanitizer tests completed for performance with issues"
fi
...
if ./performance_tests_coverage; then
    ...
    echo -e "${YELLOW}⚠${NC} Coverage tests completed for performance with issues"
fi
