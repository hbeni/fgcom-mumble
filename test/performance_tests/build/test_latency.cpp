...
public:
    ...
    virtual int getPacketsEncoded() const {
        return packets_encoded;
    }
    
private:
    mutable int packets_encoded = 0;
};
...
public:
    ...
    virtual int getPacketsTransmitted() const {
        return packets_transmitted;
    }
    
private:
    mutable int packets_transmitted = 0;
};
...
public:
    ...
    virtual int getPacketsBuffered() const {
        return packets_buffered;
    }
    
private:
    mutable int packets_buffered = 0;
};
...
TEST_F(LatencyTest, NetworkTransmissionLatency) {
    ...
    for (int size : packet_sizes) {
        ...
        auto packet_duration = std::chrono::duration_cast<std::chrono::microseconds>(packet_end_time - packet_start_time);
        double latency_ms = packet_duration.count() / 1000.0;
    ...
    double total_latency_ms = batch_duration.count() / 1000.0;
