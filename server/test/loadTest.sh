...
if [[ -z "$1" ]]; then
	echo "Usage $0 <startAtNr> <numberOfBots> <spacingInSeconds> <checkInterval> <host>"
fi
...
function spawnBot {
    bcmd="luajit test/fgcom-fakepilot.bot.lua --id=$1 --cert=$2 --key=$3 --sample=$4 --sleep=$6 --host=$7"
    echo "  cmd=$bcmd >$5 2>$5 &"
    $bcmd >$5 2>$5 &
}
...
for i in $(seq $startat $(expr $startat + $numBots - 1)); do
	...
	spawnBot $i /tmp/fgcom-bot-$i.pem /tmp/fgcom-bot-$i.key /tmp/fgcom.rec.testsample-$i.fgcs /tmp/fgcom-bot-$i.log $botsleep $host
