...
| `AUDIO_FX_NOISES` | Bool   | `0` will disable adding noise when squelch is open. FlightGear creates its own noise, so this can be disabled. | `1` |
...
Each packets *payload* consists of a comma-separated string sequence of `KEY=VALUE` pairs (empty values are to be ignored too; commas and equals-signs in values are escaped by prepending a backslash). Floats are always expected with a point as decimal-point character. Field values are restricted to 32 chars each.
...
The packet format is similar to the UDP input format: a simple `Key=Value` ASCII string. Pairs are separated using comma (commas and equals-signs in values are escaped by prepending a backslash), each packet is terminated by newline. Floats are always output with a point as decimal-point character.  
