...
namespace FGCom_TerrainEnvironmental {
    ...
    TerrainAltitudeData::TerrainAltitudeData() noexcept
        : ground_altitude(0.0), user_altitude(0.0), effective_antenna_height(0.0),
          terrain_type("unknown"), surface_material("unknown") {
    }

    bool TerrainAltitudeData::isValid() const noexcept {
        return ground_altitude >= -500.0 && ground_altitude <= 10000.0 &&
               user_altitude >= 0.0 && user_altitude <= 1000.0 &&
               effective_antenna_height >= -500.0 && effective_antenna_height <= 11000.0 &&
    ...
    TerrainAltitudeData TerrainDataProvider::getTerrainAltitude(const Coordinate& coordinate) {
        ...
        try {
            ...
            result.user_altitude = coordinate.getAltitude();
            result.effective_antenna_height = terrain_height + coordinate.getAltitude();
