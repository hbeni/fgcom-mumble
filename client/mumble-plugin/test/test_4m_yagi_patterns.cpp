...
bool test4mYagiPatternFile(const std::string& filename) {
    ...
    while (std::getline(file, line) && line_count < 100) {
        ...
        if (line_count > 8 && !line.empty() && line[0] != '#') {
            std::istringstream iss(line);
            double theta, phi, gain, h_pol, v_pol;
            if (iss >> theta >> phi >> gain >> h_pol >> v_pol) {
                has_gain_data = true;
                data_lines++;
            }
        }
    ...
    return has_frequency && has_header && has_gain_data && data_lines > 10;
}
