...
TEST_F(EnvironmentalEffectsTest, WeatherImpactRain) {
    ...
    // Calculate rain attenuation using ITU-R P.838-3 model
    double rain_attenuation_db = FGCom_PropagationPhysics::calculateRainAttenuation(frequency_mhz, distance_km);
    ...
    for (double intensity : rain_intensities) {
        // Use ITU-R P.838-3 model for different rain intensities
        double attenuation = FGCom_PropagationPhysics::calculateRainAttenuation(frequency_mhz, distance_km);
        
        EXPECT_GE(attenuation, 0.0) << "Rain attenuation should be non-negative";
        
        // For VHF frequencies, rain attenuation should be minimal
        if (frequency_mhz < 1000.0) {
            EXPECT_LT(attenuation, 1.0) << "VHF rain attenuation should be minimal";
        }
    }
}
