    ...
    if make clean && make -j$(nproc) 2>/dev/null; then
        if [ -f "build/${module_name}" ]; then
            ...
            if ./build/${module_name} 2>&1 | tee test_output.log; then
                ...
                local total_count=$(grep -c "\[  PASSED  \]\|\[  FAILED  \]" test_output.log 2>/dev/null || echo "0")
            ...
            for exe in "${module_name}" "${module_name}_tests" "test_${module_name}"; do
                if [ -f "build/$exe" ]; then
                    ...
                    if ./build/$exe 2>&1 | tee test_output.log; then
                        local test_count=$(grep -c "\[  PASSED  \]" test_output.log 2>/dev/null || echo "0")
                        local total_count=$(grep -c "\[  PASSED  \]\|\[  FAILED  \]" test_output.log 2>/dev/null || echo "0")
...
cat > /home/haaken/github-projects/fgcom-mumble/tests-passed.md << EOF
